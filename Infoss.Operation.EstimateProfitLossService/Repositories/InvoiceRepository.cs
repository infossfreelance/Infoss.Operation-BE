...
namespace Infoss.Operation.EstimateProfitLossService.Repositories
{
    ...
    {   
        ...
        public async Task<Response<ResponseInvoice>> ReadHeaderByID(string Id)
        {
            var results = new Response<ResponseInvoice>();
            try
            {
                var parameters = new DynamicParameters();
                parameters.Add("@Id", Id);
                ...
                using (var connection = new SqlConnection(connectionString))
                {
                    using (var multi = (await connection.QueryMultipleAsync("GET_INVOICE", parameters, commandType: CommandType.StoredProcedure)))
                    {
                        //var ResponseInvoice = (await multi.ReadAsync<ResponseInvoice>()).First();
                        //ResponseInvoice responseinvoice = new ResponseInvoice();
                        //responseinvoice = ResponseInvoice;
                        ResponseInvoice ResponseInvoice2 = new ResponseInvoice();
                        var customer = multi.Read<ResponseInvoice>().First();

                        var orders = multi.Read<ResponseInvoice>().First();
                        ResponseInvoice2 = customer;
                        //List<ResponseInvoice> responseinvoice = new List<ResponseInvoice>();
                        //responseinvoice = customer;

                        //results.Data = (await multi.ReadAsync<ResponseInvoice>()).ToList();
                        //results.CountData = parameters.Get<int>("@RecordCount");
                        //if (results.Data == null)
                        //{
                        //    results.Code = 204;
                        //}
                        results.data = ResponseInvoice2;
                        return results;
                    }
                }
            }
